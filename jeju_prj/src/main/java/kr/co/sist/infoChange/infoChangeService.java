...
public class infoChangeService {
    ...
    public MemberDomain getMemberInfo(String userId) {
        ...
        if (member != null && member.getBirth() != null && !member.getBirth().equals("-")) {
            try {
                ...
                System.out.println("Decrypted Birth: " + decryptedBirth);
                
                if (decryptedBirth.length() == 8) {
                    // 'yyyyMMdd' 형식을 'yyyy-MM-dd' 형식으로 변환
                    String formattedBirth = decryptedBirth.substring(0,4) + "-" 
                                          + decryptedBirth.substring(4,6) + "-" 
                                          + decryptedBirth.substring(6,8);
                    member.setBirth(formattedBirth); // '2005-11-20'
                    System.out.println("Formatted Birth: " + member.getBirth());
                }
            } catch (Exception e) {
                System.out.println("Birth decryption failed: " + e.getMessage());
    ...
    public boolean modifyInfo(MemberVO mVO) {
        ...
        System.out.println("Original Birth: " + birth);
        ...
        if (birth != null && birth.contains("-")) {
            ...
            try {
                ...
                System.out.println("Encrypted Birth: " + mVO.getBirth());
            } catch (Exception e) {
                System.out.println("Birth encryption failed: " + e.getMessage());
                // 암호화 실패 시, 수정 실패로 처리
                return false;
            }
        }
