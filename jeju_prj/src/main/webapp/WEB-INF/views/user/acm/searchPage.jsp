	...
	<main class="main-container">
		<form method="get" action="/acm/search">
			...
			<label for="matching-location"> <input type="checkbox"
				id="matching-location" name="matchingLocation"> 매칭 숙소 제외
			</label>

			<!-- 숙소 유형 선택 -->
			<h3>숙소 유형</h3>
			<label><input type="radio" name="accommodationType"
				value="호텔"> 호텔</label> <label><input type="radio"
				name="accommodationType" value="리조트"> 리조트</label> <label><input
				type="radio" name="accommodationType" value="펜션/풀빌라"> 펜션/풀빌라</label>
			<label><input type="radio" name="accommodationType"
				value="게하/한옥"> 게하/한옥</label> <label><input type="radio"
				name="accommodationType" value="캠핑/글램핑"> 캠핑/글램핑</label> <label><input
				type="radio" name="accommodationType" value="홈 빌라"> 홈 빌라</label>

			<!-- 가격 범위 -->
			<input type="range" min="0" max="500000" value="0"
				class="price-range" id="price-range" name="priceRange">
			<p id="price-text">0원 - 500,000원 이상</p>

			<!-- 공용 시설 -->
			<h3>공용 시설</h3>
			<button type="button" class="facility-btn" name="facility"
				value="레스토랑">레스토랑</button>
			<button type="button" class="facility-btn" name="facility"
				value="라운지">라운지</button>
			<button type="button" class="facility-btn" name="facility"
				value="바비큐">바비큐</button>
			<button type="button" class="facility-btn" name="facility"
				value="샤워실">샤워실</button>
			<button type="button" class="facility-btn" name="facility"
				value="주차장">주차장</button>

			<!-- 객실 내 시설 -->
			<h3>객실 내 시설</h3>
			<button type="button" class="facility-btn" name="roomFacility"
				value="무선와이파이">무선와이파이</button>
			<button type="button" class="facility-btn" name="roomFacility"
				value="에어컨">에어컨</button>
			<button type="button" class="facility-btn" name="roomFacility"
				value="금연">금연</button>
			<button type="button" class="facility-btn" name="roomFacility"
				value="TV">TV</button>
		...
		<section>
			<c:forEach var="accommodation" items="${accommodationList}">
				<div class="accommodation-item">
					<img src="${acmDetail.image}" alt="${acmDetail.name}"
						class="accommodation-image">
					<div class="accommodation-details">
						<!-- 숙소 이름을 클릭하면 hotelDetail.jsp로 이동 -->
						<h3>
							<a href="hotelDetail.jsp?hotelId=${accommodation.acm_id}"
								style="text-decoration: none; color: inherit;">
								${acmDetail.name} </a>
						</h3>
						<p>${acmDetail.address}</p>
						<p>${acmDetail.rating}★${acmDetail.reviewCount}명평가</p>
						<p>
